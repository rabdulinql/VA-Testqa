...
зафиксировать заказаееый прсиавшику товар
чтобы проанализировать разницу между тем что было заказано и что ббыло фактически доставлено
...
* Проверка заполнения цены из соглашения
	Тогда таблица "ItemList" содержит строки:
		| 'Сумма скидки' | 'N' | 'Номенклатура'            | 'Характеристика'          | 'Количество' | 'Ед. изм.' | 'Вид цены'          | 'Центр прибыли/убытков' | 'Цена'   |
		| ''             | '1' | 'Товар без характеристик' | 'Товар без характеристик' | '1,000'      | 'шт'       | 'Цена поставщика 1' | ''                      | '150,00' |
* Изменение вида цен и проверка его заполнения при перевыборе соглашения
